...
function commentDisplay($mainPosts, $parentID=0, $level=0)
{
  ...
  foreach ($posts as $key => $post) {
    ...
    $output .= sprintf('<div class="commentMeta"><small class="voteCount commentVotes" data-postid="%s">voted: %s</small> | <small class="commentDate">%s</small> | <small class="commentLink"><a href="?postID=%s">permalink</a></small></div></div>', $post['postID'], $post['voteCount'], $postDate, $post['postID']);
    if ($hasComments) {
